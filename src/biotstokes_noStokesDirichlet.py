...
class BiotStokesProblem(object):
    ...
    def get_solver(self):
        ...
        def compute_RHS(dp_prev, pp_prev, neumann_bcs, t):
            ...
            up_e, pp_e, dp_e, uf_e, pf_e = self.exact_solution()
            uf_e.t = t
            pf_e.t = t
            ...
            L_stokes_neumann = 0
            for subdid in self._dirichlet_bcs['stokes']:
                L_stokes_neumann += Constant(-2*mu_f)*inner(dot(sym(grad(uf_e)), nf), vf)*dsStokes(subdid)
                L_stokes_neumann += Constant(1.)*inner(pf_e*nf, vf)*dsStokes(subdid)

            L = [
                L_Cp_vp + Constant(mu_p/K)*S_vp,
                -(bpp + S_wp),
                Co(1/dt) * (L_Cp_ep + S_ep + L_BJS_ep),
                S_vf + L_BJS_vf - L_stokes_neumann,
                -S_wf,
                L_mult
